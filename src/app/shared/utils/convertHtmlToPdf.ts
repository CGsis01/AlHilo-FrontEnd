...
export class ConvertHtmlToPdf {
    async convertirHtmlToPdf(elementId: string, fileName: string): Promise<Blob> {
        ...
        const origEls = [element, ...Array.from(element.querySelectorAll<HTMLElement>('*'))];
        ...
        const blob = await html2pdf().set(opciones).from(element).outputPdf('blob').then();
