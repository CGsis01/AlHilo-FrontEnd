...
export class RepairApiService {
  ...
  getAll(filters?: RepairFilters): Observable<Repair[]> {
    ...
    return this.apiService
      .get<Repair[]>(this.endpoint, params)
  ...
  getPaginated(
  ...
  ): Observable<PaginatedResponse<Repair>> {
    ...
    return this.apiService
      .get<PaginatedResponse<Repair>>(this.endpoint, params)
      .pipe(
        map((response) => {
  ...
  create(repairData: CreateRepairRequest): Observable<Repair> {
    return this.apiService
      .post<Repair>(this.endpoint, repairData)
      .pipe(map((r) => this.mapRepair(r)));
  }

  update(id: string, repairData: UpdateRepairRequest): Observable<Repair> {
    return this.apiService
      .put<Repair>(`${this.endpoint}/${id}`, repairData)
      .pipe(map((r) => this.mapRepair(r)));
  }

  patch(
    id: string,
    repairData: Partial<UpdateRepairRequest>,
  ): Observable<Repair> {
    return this.apiService
      .patch<Repair>(`${this.endpoint}/${id}`, repairData)
      .pipe(map((r) => this.mapRepair(r)));
  }
  ...
  updateStatus(update_status_params: UpdateStatusRequest): Observable<Repair> {
    return this.apiService
      .post<Repair>(`${this.endpoint}/update-status`, update_status_params)
      .pipe(map((r) => this.mapRepair(r)));
  }

  assignRepairGarments(
    assign_params: AssignRepairGarments,
  ): Observable<Repair> {
    return this.apiService
      .post<Repair>(
        `${this.endpoint}/${assign_params.repair_id}/assign-garments`,
        assign_params,
      )
      .pipe(map((r) => this.mapRepair(r)));
  }

  assignSingleRepairItem(
    assign_params: AssignRepairItem,
  ): Observable<RepairItem> {
    return this.apiService
      .post<any>(
        `${this.endpoint}/repair-items/${assign_params.repair_item_id}/assign`,
        assign_params,
      )
      .pipe(map((i) => this.mapRepairItemResponse(i)));
  }

  updateSingleRepairItemStatus(
    updateParams: UpdateRepairItemStatusRequest,
  ): Observable<Repair> {
    return this.apiService
      .post<Repair>(
        `${this.endpoint}/repair-items/${updateParams.repair_item_id}/update-status`,
        updateParams,
      )
      .pipe(map((r) => this.mapRepair(r)));
  }
  ...
  assignToSeamstress(assign_params: AssignRepairRequest): Observable<Repair> {
    ...
    return this.apiService
      .post<Repair>(`${this.endpoint}/assign`, assign_params)
      .pipe(map((r) => this.mapRepair(r)));
  }
  ...
  addRepairItem(
    repairId: string,
    item: RepairItemRequest,
  ): Observable<RepairItem> {
    return this.apiService
      .post<any>(`${this.endpoint}/${repairId}/items`, item)
  ...
  updateRepairItem(
    repairId: string,
    itemId: string,
    item: Partial<RepairItemRequest>,
  ): Observable<RepairItem> {
    return this.apiService
      .put<any>(`${this.endpoint}/${repairId}/items/${itemId}`, item)
      .pipe(map((i) => this.mapRepairItem(repairId, i)));
  }

  removeRepairItem(repairId: string, itemId: string): Observable<void> {
    return this.apiService.delete<void>(
      `${this.endpoint}/${repairId}/items/${itemId}`,
    );
  }
  ...
  addComment(
    ...
    comment: string,
    createdBy: string,
  ): Observable<RepairComment> {
    return this.apiService.post<RepairComment>("/repair-comments/", {
      repair_id: repairId,
      comment,
  ...
  private buildFilterParams(filters?: RepairFilters): HttpParams {
    ...
    if (filters) {
      if (filters.repair_status_id)
        params = params.set("repair_status_id", filters.repair_status_id);
      if (filters.repair_type_id)
        params = params.set("repair_type_id", filters.repair_type_id);
      if (filters.created_by_id)
        params = params.set("created_by_id", filters.created_by_id);
      if (filters.client_id)
        params = params.set("client_id", filters.client_id);
      if (filters.customer_phone)
        params = params.set("customer_phone", filters.customer_phone);
      if (filters.search) params = params.set("search", filters.search);
  ...
  private mapRepair(repair: any): Repair {
        ...
        ? repair.repair_items.map(
            (item: any): RepairItem => ({
              ...
              repairTypes: Array.isArray(item.repair_item_repair_types)
                ? item.repair_item_repair_types.map((rirt: any) => ({
                    ...
                    commissionPercentage:
                      rirt.repair_type.commission_percentage,
                    repairComplexity: rirt.repair_type.repair_complexity
                      ? {
                          ...
                          laborMultiplier:
                            rirt.repair_type.repair_complexity.labor_multiplier,
                          timeMultiplier:
                            rirt.repair_type.repair_complexity.time_multiplier,
                          storeId: rirt.repair_type.repair_complexity.store_id,
                          isActive:
                            rirt.repair_type.repair_complexity.is_active,
                          createdAt:
                            rirt.repair_type.repair_complexity.created_at,
                          updatedAt:
                            rirt.repair_type.repair_complexity.updated_at,
                        }
                      : undefined,
              ...
              repairStatus: item.repair_status
                ? {
                    id:
                      item.repair_status.repair_status_id ||
                      item.repair_status.id,
    ...
    return {
      ...
      finalPrice: items
        ? items.reduce(
            (sum, i) => sum + (Number(i.finalPrice ?? i.estimatedPrice) || 0),
            0,
          )
      ...
      actualDeliveryDate: repair.actual_delivery_date
        ? new Date(repair.actual_delivery_date)
        : undefined,
      notes: repair.notes,
      items,
  ...
  getRepairItemsBySeamstress(seamstressId: string): Observable<RepairItem[]> {
    return this.apiService
      .get<any[]>(`${this.endpoint}/seamstress/${seamstressId}/repair-items`)
