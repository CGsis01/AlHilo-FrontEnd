...
export class RepairDetailComponent implements OnInit {
  ...
  getRepairStatus(statusName: string): RepairStatus {
    return this.repairStatuses().filter(
  ...
  updateStatus(newStatus: RepairStatus): void {
    ...
    this.repairUseCases
      .updateRepairStatus(this.repair.id, newStatus)
      .subscribe({
        next: (updatedRepair) => {
              ...
              .subscribe((result) => {
                if (result.success) {
                  this.toastService.show(
                    "Notificación WhatsApp enviada: reparación lista",
                    "success",
                  );
                } else {
                  this.toastService.show(
  ...
  loadSeamstresses(): void {
    ...
    this.userUseCases
      .getUsersByRole([
        { code: UserRoleCode.SEAMSTRESS },
        { code: UserRoleCode.HEADSEWING },
      ] as UserRole[])
      .subscribe({
        next: (users) => {
  ...
  async confirmAssignment(): Promise<void> {
    if (!this.repair || !this.selectedSeamstress) return;

    this.repairUseCases
      .assignRepairToSeamstress(this.repair.id, this.selectedSeamstress)
      .subscribe({
        next: async (updatedRepair) => {
          this.repair = updatedRepair;
          this.updateStatus(
            this.repairStatuses().filter(
              (s) => s.name === RepairStatusEnum.IN_PROGRESS,
            )[0],
          );
          ...
          this.whatsappApiService
            ...
            .subscribe((result) => {
              if (result.success) {
                this.toastService.show(
                  "Notificación WhatsApp enviada: reparación en progreso",
                  "success",
                );
              } else {
                this.toastService.show(
  ...
  async confirmJobReview(): Promise<void> {
    ...
    this.repairUseCases
      ...
      .subscribe({
        ...
        error: (error) => {
          this.toastService.show(
            "Error al agregar el comentario: " + error.message,
            "error",
          );
        },
      });
  ...
  confirmPayment(): void {
    ...
    this.repairUseCases
      .updateRepairStatus(this.repair.id, deliveredStatus)
      .subscribe({
        next: (updatedRepair) => {
          ...
          if (this.paymentType === "mixed") {
            if (mixedTotal < remaining) {
              this.toastService.show(
                "El pago mixto debe cubrir el saldo restante.",
                "error",
              );
            ...
            if (mixedTotal > remaining) {
              this.toastService.show(
                "El pago mixto no puede ser mayor al saldo restante.",
                "error",
              );
              this.handleRepairUpdated(updatedRepair);
            ...
            if (mixedCardPaid > remaining) {
              this.toastService.show(
                "El pago con tarjeta no puede ser mayor al saldo restante.",
                "error",
              );
              this.handleRepairUpdated(updatedRepair);
            ...
            if (mixedCardPaid > 0 && !this.voucherId.trim()) {
              this.toastService.show(
                "Ingresa el ID del voucher para registrar el pago con tarjeta.",
                "error",
              );
              this.handleRepairUpdated(updatedRepair);
          ...
          if (
            ...
            if (
              ...
              this.toastService.show(
                "La reparación se actualizó, pero no se pudieron mapear los tipos de pago de la reparación.",
                "error",
              );
            } else if (needsCash && !paymentTypeCash) {
              this.toastService.show(
                "La reparación se actualizó, pero no se pudo mapear el tipo de pago en efectivo.",
                "error",
              );
            } else {
              this.toastService.show(
                "La reparación se actualizó, pero no se pudo mapear el tipo de pago con tarjeta.",
                "error",
              );
            }
          ...
          forkJoin(paymentRequests).subscribe({
            ...
            error: () => {
              this.toastService.show(
                "La reparación se actualizó, pero no se pudo registrar el pago.",
                "error",
              );
              this.handleRepairUpdated(updatedRepair);
            },
  ...
  get hasMixedCardPayment(): boolean {
    return (
      this.paymentType === "mixed" &&
      this.getNumericAmount(this.mixedCardAmount) > 0
    );
  }
  ...
  get hasPaidMixedCardPayment(): boolean {
    return (
      ...
      this.getNumericAmount(this.paidMixedCardAmount) > 0
    );
  }
  ...
  private async triggerWorkOrderPrint(
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      try {
        ...
        this.toastService.show(
          "No se pudieron imprimir todos los tickets",
          "error",
        );
        this.closeTicket();
        return;
